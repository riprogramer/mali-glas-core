...
class Blockchain {
    ...
    getDifficulty(index) {        
        ...
        return Config.bitcoin.getDifficulty(this.blocks, index);        
    }
    
    nextBlockDifficulty() {        
        // Calculates the difficulty based on the index since the difficulty value increases every X blocks.
        return Config.bitcoin.nextBlockDifficulty(this.blocks);        
    }
    ...
    checkBlock(newBlock, previousBlock, referenceBlockchain = this.blocks) {
        ...
        let sumOfInputsAmount = R.sum(R.flatten(R.map(R.compose(R.map(R.prop('amount')), R.prop('inputs'), R.prop('data')), newBlock.transactions))) + Config.bitcoin.getMiningReward(this.getAllBlocks());
