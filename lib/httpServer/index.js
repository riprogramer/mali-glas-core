...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.get('/blockchain/difficulty', (req, res) => {
        		let blockchainDifficulty = blockchain.getDifficulty()
        		res.status(200).send({difficulty: blockchainDifficulty});
        });
        
        this.app.get('/blockchain/reward', (req, res) => {
	    		res.status(200).send({reward: Config.bitcoin.getMiningReward(blockchain.getAllBlocks())});
	    });
        ...
        this.app.post('/miner/block', (req, res, next) => {
        		let nextBlock = miner.getNextBlock(req.body.rewardAddress, req.body.feeAddress);
        		res.status(200).send({nextBlock});
        });
